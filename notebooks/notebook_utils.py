...
from utils_ import prettify_name, pad_frames
...
def _create_strip_batch_sigma(inst, mode, layer, latents, x_comp, z_comp, act_stdev, lat_stdev, act_mean, lat_mean, sigma, layer_start, layer_end, num_frames, center):    
    ...
    sigma_range = torch.from_numpy(sigma_range).float().cuda()
            ...
            with torch.no_grad():
                ...
                if mode in ['latent', 'both']:
                    ...
                    for i in range(layer_start, layer_end):
                        ...
                        # z[:,i,:] = z[:,i,:] - zeroing_offset_lat[:,i,:] + delta

                img_batch = inst.model.sample_np(z)
                # w = torch.tensor(z).cuda() if not torch.is_tensor(z) else z 
                # img_batch = inst.synthesis(w, noise_mode='const', force_fp32 = True)
                # img_batch = img_batch.permute(0, 2, 3, 1).clamp(0.0, 1.0).cpu().detach().numpy()
                if img_batch.ndim == 3:
